...
El refresco de mercado se configura entre 30 y 86.400 segundos; por defecto 60. GoldAPI tiene un timeout de seis segundos y oro/plata se solicitan en paralelo. Redis conserva último mercado con fecha. Ante fallo se devuelve último dato o fixing de emergencia con `stale=true`. La CDN puede conservar precios durante 30 segundos y servirlos caducados otros 30 mientras revalida, así que un cambio de fórmulas tarda como mucho un minuto en verse. El portal de clientes vuelve a pedir la tarifa al ritmo del refresco configurado (acotado entre 30 segundos y 10 minutos) y al volver a la pestaña. La web actual introduce variaciones sintéticas adicionales: es un problema pendiente de INT-08, no una fuente de mercado.
